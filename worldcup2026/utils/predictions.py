...
from utils.deadline import is_prediction_open
...
def upsert_match_prediction(
    ...
    if room and match.kickoff_time:
        match_dict = {"status": match.status, "kickoff_time": match.kickoff_time}
        room_dict  = {"deadline_type": room.deadline_type, "fixed_window_hours": room.fixed_window_hours or 24}
        open_pred, reason = is_prediction_open(match_dict, room_dict)
        if not open_pred:
            return False, f"Prediction deadline has passed: {reason}"
