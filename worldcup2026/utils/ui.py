...
/* ── Hide non-essential Streamlit chrome — keep stHeader + stToolbar visible
   because the sidebar expand button is nested inside them ── */
#MainMenu, footer,
...
/* ── Horizontal Radio (Segmented Control for Predictions) ── */
[data-testid="stAppViewContainer"] .main [data-testid="stRadio"] > div[role="radiogroup"][aria-orientation="horizontal"] {
    display: flex !important;
    width: 100% !important;
    background: var(--surface2) !important;
    border: 1px solid var(--border) !important;
...
[data-testid="stAppViewContainer"] .main [data-testid="stRadio"] > div[role="radiogroup"][aria-orientation="horizontal"] > label {
    flex: 1 !important;
    margin: 0 !important;
...
[data-testid="stAppViewContainer"] .main [data-testid="stRadio"] > div[role="radiogroup"][aria-orientation="horizontal"] > label:last-child {
    border-right: none !important;
}
[data-testid="stAppViewContainer"] .main [data-testid="stRadio"] > div[role="radiogroup"][aria-orientation="horizontal"] > label:hover {
    background: var(--surface3) !important;
}
[data-testid="stAppViewContainer"] .main [data-testid="stRadio"] > div[role="radiogroup"][aria-orientation="horizontal"] > label:has(input:checked) {
    background: linear-gradient(180deg, var(--primary) 0%, var(--primary-dk) 100%) !important;
    color: #FFFFFF !important;
    box-shadow: inset 0 2px 5px rgba(0,0,0,0.2) !important;
}
/* Hide the radio circles */
[data-testid="stAppViewContainer"] .main [data-testid="stRadio"] > div[role="radiogroup"][aria-orientation="horizontal"] span[data-baseweb="radio"],
[data-testid="stAppViewContainer"] .main [data-testid="stRadio"] > div[role="radiogroup"][aria-orientation="horizontal"] div:first-child {
    display: none !important;
}
...
.wc-quicknav-card:hover {
    border-color: var(--primary);
    background: var(--surface3);
    transform: translateY(-2px);
    box-shadow: 0 6px 20px rgba(225,29,46,0.18);
}
.wc-quicknav-card .wc-qn-label {
...
def format_kickoff(dt):
    ...
    try:
        # If it's a string, handle the UTC 'Z' suffix
        if isinstance(dt, str): 
            dt = datetime.fromisoformat(dt.replace("Z", "+00:00"))
        
        # API returns UTC. Portugal (Lisbon) is currently WEST (UTC+1).
        # We add 1 hour to match Portugal's local time.
...
def page_header(title, subtitle="", eyebrow=""):
    ...
    # Render the fixed top bar on every page
    render_top_bar()
